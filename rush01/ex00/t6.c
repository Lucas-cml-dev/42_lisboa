...
/*                                                +#+#+#+#+#+   +#+           */
/*   Created: 2025/03/09 13:30:20 by lcarvalh          #+#    #+#             */
/*   Updated: 2025/03/09 13:30:25 by lcarvalh         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
// Função para verificar a consistência das restrições de visibilidade
int	is_consistent(int *constraints)
{
	...
	// Verifica se as restrições de visibilidade são possíveis
	// Não pode exigir mais visibilidade do que o número de elementos em uma linha/coluna (SIZE)
	i = 0;
	while (i < 4)  // Restrições superiores (colunas)
	{
	...
	i = 4;
	while (i < 8)  // Restrições inferiores (colunas)
	{
		if (constraints[i] > SIZE)
			return (0);
		i++;
	...
	i = 8;
	while (i < 12)  // Restrições laterais à esquerda (linhas)
	{
		if (constraints[i] > SIZE)
			return (0);
		i++;
	...
	i = 12;
	while (i < 16)  // Restrições laterais à direita (linhas)
	{
		if (constraints[i] > SIZE)
			return (0);
		i++;
	...
	return (1);  // As restrições são consistentes e válidas
}
...
int	check_constraints(int grid[SIZE][SIZE], int *constraints)
{
	...
	while (i < SIZE)
	{
		// Verifica as visibilidades das linhas
		int j = 0;
		while (j < SIZE)
		{
			row[j] = grid[i][j];
